...
async def update_data():
    try:
        ...
        if insight and ai_signal_label and ai_reason_markdown:
            ...
            if signal == 'COMPRA':
                ai_card.classes(remove='border-zinc-800 border-rose-500/50', add='border-emerald-500/50 shadow-[0_0_20px_rgba(16,185,129,0.1)]')
                ai_signal_label.classes(remove='text-zinc-500 text-rose-500 text-amber-500', add='text-emerald-400')
                if ai_icon_container: ai_icon_container.classes(remove='bg-zinc-800 bg-rose-500/10', add='bg-emerald-500/10 text-emerald-400')
            elif signal == 'VENDA':
                ...
                if ai_icon_container: ai_icon_container.classes(remove='bg-zinc-800 bg-emerald-500/10', add='bg-rose-500/10 text-rose-400')
            else:
...
async def start_bot():
    ...
    ui.notify('Iniciando Sistema SpotBot Pro...')
    ...
    with ui.column().classes('w-full h-screen items-center justify-center bg-black'):
        with ui.card().classes('w-80 p-8 bg-zinc-950 border border-zinc-800 shadow-2xl items-center gap-6 rounded-xl'):
            with ui.column().classes('items-center gap-2'):
                ...
                ui.label('SPOTBOT PRO').classes('text-xl font-bold tracking-tight text-white')
...
async def index():
    ...
    ui.add_head_html('''
        <style>
            ...
            .zinc-panel { background: #09090b; border: 1px solid #27272a; }
    ...
    with ui.header().classes('h-14 bg-black/80 backdrop-blur border-b border-zinc-800 flex items-center px-4 justify-between z-50'):
        with ui.row().classes('items-center gap-2'):
            ...
            with ui.column().classes('gap-0'):
                ui.label('SPOTBOT PRO').classes('text-sm font-bold tracking-tight text-white leading-none')
             ...
             with ui.card().classes('w-full h-[65vh] p-0 rounded-none bg-black border-b border-zinc-800 gap-0 shadow-none relative'):
                 ...
                 with ui.element('div').classes('w-full h-full'):
                     candle_chart = ui.echart({
                        ...
                        'grid': [{'left': '50', 'right': '20', 'top': '40', 'height': '60%'}, {'left': '50', 'right': '20', 'top': '75%', 'height': '15%'}],
                        ...
                        'xAxis': [{'type': 'category', 'data': [], 'gridIndex': 0, 'axisLine': {'lineStyle': {'color': '#52525b'}}}, {'type': 'category', 'data': [], 'gridIndex': 1, 'axisLabel': {'show': False}, 'axisTick': {'show': False}, 'axisLine': {'show': False}}],
                        'yAxis': [{'type': 'value', 'scale': True, 'gridIndex': 0, 'splitLine': {'lineStyle': {'color': '#18181b'}}, 'position': 'right'}, {'type': 'value', 'scale': True, 'gridIndex': 1, 'splitLine': {'show': False}, 'axisLabel': {'show': False}, 'axisTick': {'show': False}}],
                        'series': [
...
def start_dashboard():
    ...
    ui.run(title='SpotBot Pro | Terminal', dark=True, reload=False, port=port, storage_secret=secret)
