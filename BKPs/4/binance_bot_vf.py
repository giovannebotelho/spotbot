...
sell_pressure_history = deque(maxlen=10)
...
async def adjust_and_place_oco_order(client, symbol, quantity, tick_size, min_price_move):
    ...
    for attempt in range(max_attempts):
        ...
        if current_price < 1:
            ...
            lucro_multiplier = 1.0035
            stop_loss_multiplier = 0.9975
        ...
        try:
            ...
            if attempt < max_attempts - 1:
                ...
                await asyncio.sleep(1.5)  # Breve pausa antes de tentar novamente
...
async def get_closes(client, symbol, interval='15m', limit=10):
    klines = await client.get_klines(symbol=symbol, interval=interval, limit=limit)
    closes = [float(kline[4]) for kline in klines]
    return closes

def calculate_macd(closes, slow=20, fast=12, signal=9):
...
async def main():
    ...
    async with bsm.user_socket() as um:
        while True:
                ...
                if rsi < 30 or (macd_current > signal_line_current and closes[-1] < lower_band):
                    ...
                    while True:
                        ...
                        if msg['e'] == 'listStatus' and msg['s'] == symbol and msg['g'] == oco_order['orderListId']:
                            if msg['l'] == 'ALL_DONE':
                                # Recuperar detalhes da ordem OCO
                                oco_order_details = await client.get_oco_order(orderListId=oco_order['orderListId'])

                                # Analisar as subordens para encontrar os preços de execução
                                for order in oco_order_details['orders']:
                                    if order['status'] == 'FILLED':  # A ordem foi totalmente preenchida
                                        executed_price = order['price']
                                        # Determinar se a ordem executada foi a de lucro ou stop loss
                                        if float(executed_price) == float(oco_order['price']):
                                            print(f"OCO Order completed at \033[32mprofit\033[0m price: \033[32m{executed_price}\033[0m")
                                        elif float(executed_price) == float(oco_order['stopLimitPrice']):
                                            print(f"OCO Order completed at \033[31mstop\033[0m loss price: \033[31m{executed_price}\033[0m")
            ...
            await asyncio.sleep(0.8)  # Short pause before checking for new orders or balance updates
