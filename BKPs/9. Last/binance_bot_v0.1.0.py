...
# Variáveis globais
STOP_LOSS_LIMIT = 2
SHORT_PAUSE = 600  # Pausa de 10 minutos (em segundos)
LONG_PAUSE = 1800  # Pausa de 30 minutos (em segundos)
stop_loss_count = 0
...
def synchronize_time():
    ...
    if last_sync_time is None or datetime.now() - last_sync_time > timedelta(days=7):
        try:
        ...
        print("🟡 Sincronização de relógio não necessária. Última feita há menos de 7 dias.")
...
async def get_order_book(client, symbol, depth=12): # 🟣🟣
...
sell_pressure_history = deque(maxlen=12) # 🟣🟣

async def calculate_moving_average_sell_pressure(client, symbol, depth=12): # 🟣🟣
...
async def adjust_and_place_oco_order(client, symbol, quantity, tick_size, min_price_move):
    ...
    for attempt in range(max_attempts):
        ...
        if current_price < 1:
            ...
            lucro_multiplier = 1.0023 # 🟣🟣
            stop_loss_multiplier = 0.9977 # 🟣🟣
...
def calculate_rsi(closes, period=15): # 🟣🟣
...
async def get_closes(client, symbol, interval='15m', limit=15): # 🟣🟣
    klines = await client.get_klines(symbol=symbol, interval=interval, limit=limit)
    closes = [float(kline[4]) for kline in klines]
    return closes

async def get_volumes(client, symbol, interval='15m', limit=15): # 🟣🟣
    klines = await client.get_klines(symbol=symbol, interval=interval, limit=limit)
...
def calculate_bollinger_bands(closes, period=15, num_std=2): # 🟣🟣
    closes_series = pd.Series(closes)
...
def calculate_volume_moving_average(volumes, period=15): # 🟣🟣
...
async def check_trend(client, symbol, interval='15m', short_period=20, long_period=50): # 🟣🟣
...
def process_order_details(limit_order_details, stop_order_details, price, executed_qty, quantia_usdt_investimento_inicial):
    """
    Processa os detalhes das ordens OCO para determinar o resultado e atualiza o saldo atual.
    """    
    # Determine o resultado da ordem
...
def create_data_row(order_count, saldo_inicial_usdt, quantia_usdt_investimento_inicial, symbol, executed_qty, price_rounded, purchase_timestamp, lucro_alvo, stop_loss, stop_limit, order_result, oco_timestamp, trade_result, total_difference, saldo_atual_usdt, rsi_value):
    return {
        ...
        "RSI Utilizado": rsi_value  # Adicionando RSI utilizado na ordem
    }
...
def is_hammer(candle):
    ...
    # Critérios de um martelo: pequeno corpo, pouca ou nenhuma sombra superior, sombra inferior longa
...
def is_shooting_star(candle):
    ...
    # Critérios de uma estrela cadente: pequeno corpo, longa sombra superior, pouca ou nenhuma sombra inferior
...
def is_bullish_engulfing(candle1, candle2):
    """
    Verifica se o padrão de engolfo de alta está presente.
    candle1: candle anterior
    candle2: candle atual (deve ser de alta e engolfar completamente o candle1)
    """
    open1, close1 = float(candle1[1]), float(candle1[4])
    open2, close2 = float(candle2[1]), float(candle2[4])

    # Verifica se o candle2 é de alta e engolfa completamente o candle1
    return (close1 < open1) and (close2 > open2) and (close2 > open1) and (open2 < close1)

async def check_candle_patterns(client, symbol):
    # Obter os últimos candles necessários para análise de padrão
    klines = await client.get_klines(symbol=symbol, interval='15m', limit=20)
    ...
    if hammer:
        decision = 'buy'
        msg = "🟢 Martelo detectado, considerando posição de \033[1;32compra!\033[0m"
    elif shooting_star:
        decision = 'sell'
        msg = "🔴 Estrela Cadente detectada, considerando posição de \033[1;31venda!\033[0m"
    elif bullish_engulfing:
        decision = 'buy'
        msg = "🟢 Engolfo de alta detectado, considerando posição de \033[1;32compra!\033[0m"
...
def should_buy(rsi, trend_is_up, candle_decision, macd_current, signal_line_current, last_close, lower_band):
    is_rsi_low_level0 = rsi < 25  # 🟣🟣
    is_rsi_low_level1 = rsi < 30  # 🟣🟣
    is_rsi_low_level2 = rsi < 35  # 🟣🟣
    is_macd_bullish = macd_current > signal_line_current and last_close < lower_band
    return is_rsi_low_level0 or (is_rsi_low_level1 and trend_is_up) or (is_rsi_low_level2 and candle_decision == 'buy') or is_macd_bullish

def should_sell(rsi, trend_is_up, candle_decision, macd_current, signal_line_current, last_close, lower_band):
    is_rsi_high = rsi > 70  # 🟣🟣
    is_macd_bearish = macd_current < signal_line_current and last_close > lower_band
    return (is_rsi_high and not trend_is_up) or (is_rsi_high and candle_decision == 'sell') or is_macd_bearish

async def run_bot():
    global restart_attempts, stop_loss_count, last_stop_loss_time, block_active, pause_end_time, quantia_usdt_investimento_inicial
    ...
    try:
        ...
        if symbol == "ADAUSDT" or symbol == "DOGEUSDT":
            print(f"📊 MACD Atual para {symbol}: \033[1;33m{macd_current:.4f}\033[0m, Linha de sinal: \033[1;33m{signal_line_current:.4f}\033[0m")
        else:
        ...
        if symbol == "ADAUSDT" or symbol == "DOGEUSDT":
            print(f"📊 Bandas de Bollinger para {symbol}: Inferior: \033[1;31m${lower_band:.4f}\033[0m, Média: \033[1;33m${middle_band:.4f}\033[0m, Superior: \033[1;32m${upper_band:.4f}\033[0m")
        else:
        ...
        async with bsm.user_socket() as um:
            while True:
                if await should_place_order(client, symbol):
                    ...
                    candle_decision = await check_candle_patterns(client, symbol)  # Chamada da função de padrões de candle
            
                    if should_buy(rsi, trend_is_up, candle_decision, macd_current, signal_line_current, closes[-1], lower_band):
                        print("### ------------------------- ###")
                            ...
                            if msg.get('e') == 'listStatus' and msg.get('s') == symbol and msg.get('g') == oco_order['orderListId']:
                                if 'ALL_DONE' in msg.get('l'):
                                    ...
                                    if order_result:
                                        ...
                                        data_row = create_data_row(order_count, saldo_inicial_usdt, quantia_usdt_investimento_inicial, symbol, executed_qty, price_rounded, purchase_timestamp, lucro_alvo, stop_loss, stop_limit, order_result, oco_timestamp, trade_result, total_difference, saldo_atual_usdt, rsi)
                    ...
                    if should_sell(rsi, trend_is_up, candle_decision, macd_current, signal_line_current, closes[-1], lower_band):
                        ticker = await client.get_symbol_ticker(symbol=symbol)
                        current_price = float(ticker['price'])
                        if current_price < 1:
