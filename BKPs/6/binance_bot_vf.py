...
def synchronize_time():
    try:
        ...
        message = "Relógio do sistema sincronizado com sucesso."
    ...
    for attempt in range(max_attempts):
        ...
        try:
            ...
            if symbol == "DOGEUSDT" or symbol == "ADAUSDT":
                ...
                message = f"OCO Order placed. Coin: <b>{symbol}</b>. ID: <b>{order_list_id}</b>. Profit: <b>${lucro_alvo:.4f}</b>, Stop Price: <b>${stop_loss:.4f}</b>, Stop Limit: <b>${stop_limit:.4f}</b>"
                send_telegram_message(bot_token, chat_id, message)
            else:
                ...
                message = f"OCO Order placed. Coin: <b>{symbol}</b>. ID: <b>{order_list_id}</b>. Profit: <b>${lucro_alvo:.2f}</b>, Stop Price: <b>${stop_loss:.2f}</b>, Stop Limit: <b>${stop_limit:.2f}</b>"
                send_telegram_message(bot_token, chat_id, message)
...
async def run_bot():
    ...
    try:
        ...
        print("\n\033[1;4;34mSeja bem-vindo(a) ao Gio Binance Bot - v0.0.1\033[0m")
        message = "<b><u>Seja bem-vindo(a) ao Gio Binance Bot - v0.0.1</u></b>"
        send_telegram_message(bot_token, chat_id, message)
        ...
        if quantia_usdt_investimento_inicial is None:
            ...
            print("\033[0m")
            message = f"Initial USDT amount invested: <b>${quantia_usdt_investimento_inicial}</b>"
            send_telegram_message(bot_token, chat_id, message)
        else:
            print(f"Using saved initial USDT amount: {quantia_usdt_investimento_inicial}")
            message = f"Using saved initial USDT amount: <b>{quantia_usdt_investimento_inicial}</b>"
            send_telegram_message(bot_token, chat_id, message)
