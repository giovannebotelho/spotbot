...
def create_data_row(order_count, saldo_inicial_usdt, quantia_usdt_investimento_inicial, symbol,
    ...
    return {
        ...
        "Multiplicador de Lucro (Preço < 1)": OCO_CONFIG.get('target_profit_percent', 0.025),
        "Multiplicador de Stop Loss (Preço < 1)": OCO_CONFIG.get('stop_loss_percent', 0.02),
        "Multiplicador de Lucro (Preço >= 1)": OCO_CONFIG.get('target_profit_percent', 0.025),
        "Multiplicador de Stop Loss (Preço >= 1)": OCO_CONFIG.get('stop_loss_percent', 0.02),
        "Limiar de pressão de venda": TRADING_CONFIG.get('sell_pressure_threshold', 0.65),
        "Período de cálculo do RSI e médias móveis": TRADING_CONFIG.get('period', 14),
        "Número desvios padrões Bollinger": TRADING_CONFIG.get('num_std', 2.0),
        "Período curto cálculo média móvel <> tendência": TRADING_CONFIG.get('short_period', 9),
        "Período longo cálculo média móvel <> tendência": TRADING_CONFIG.get('long_period', 21),
        "Limite dados históricos para recuperar de uma vez": TRADING_CONFIG.get('limit', 100),
        "Profundidade do livro de ofertas": TRADING_CONFIG.get('depth', 20),
        "Tamanho máximo para deque": TRADING_CONFIG.get('maxlen', 10),
        "Condicional volume": TRADING_CONFIG.get('volume_avg', 0),
